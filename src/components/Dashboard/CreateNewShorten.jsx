...
const CreateNewShorten = ({ setOpen, refetch }) => {
  ...
  const createShortUrlHandler = async (data) => {
    setLoading(true);
    try {
        const { data: res } = await apiClient.post("/api/urls/shorten", data, {
            headers: {
  ...
  return (
    ...
    <form
      ...
      >
        ...
        <button
          className="bg-blue-500 font-semibold text-white w-32  bg-custom-gradient  py-2  transition-colors  rounded-md my-3"
          type="text"
        >
